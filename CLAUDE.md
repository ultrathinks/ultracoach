...
미들웨어(`src/middleware.ts`)가 `/api/auth` 외 모든 API에 JWT 인증 적용. `auth.config.ts`(Edge 호환)와 `auth.ts`(DB adapter 포함) 분리 구조. 소유권 확인이 필요한 route(`/api/sessions`, `/api/sessions/[id]/feedback`)는 handler 내부에서 추가로 `auth()` 호출.
